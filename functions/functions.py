    ...
    def calculate(
        ...
        if value:
            self.value = (
                ...
                / 2
                * sum(
                    np.square(self.set_starting_point[1::2])
    ...
    def calculate(
        ...
        if value:
            self.value = sum(
                ...
                self.starting_point[0] + self.set_starting_point[-1]
            )
...
class Hager:
    ...
    def calculate(
        ...
        if hessian:
            self.hessian = np.diag(
                np.exp(sel.starting_point)
                - np.arange(1, n + 1) * self.starting_point ** (-3 / 2) / 2
            )
